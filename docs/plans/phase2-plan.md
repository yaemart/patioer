...
> 节奏原则：先打通测试与观测基线，再并行执行隔离/并发/限流验证，最后收口文档与验收。

##### Day 1 — 基线冻结 + 测试工装准备

**目标：** 固定 Sprint 6 验证基线，避免中途代码漂移导致结果不可复现。  
**对应任务：** 6.1 / 6.2 / 6.5 / 6.6 的前置

**关键任务：**
- 冻结验证分支与依赖版本，记录 commit hash 与测试环境参数（PG / Redis / Node / pnpm）。
- 建立 Sprint 6 测试矩阵文档：AC-P2-15~20 映射到测试用例与命令。
- 整理租户与平台测试夹具：Tenant A（Shopify+Amazon）、Tenant B（TikTok+Shopee）、并发 10 租户模板。
- 新增统一测试入口脚本（隔离、并发、限流、覆盖率分组执行）。

**验收：**
- [ ] 基线信息已写入 `docs/ops/sprint6-validation-baseline.md`
- [ ] 测试矩阵可追踪到 AC-P2-15~20
- [ ] 测试夹具可一键初始化

**产出：** 可复现的 Sprint 6 验证基线 + 测试入口

---

##### Day 2 — 多平台隔离测试（数据面）

**目标：** 完成租户 A/B 数据不可见验证（AC-P2-15）。  
**对应任务：** 6.1

**关键任务：**
- 编写隔离集成测试：订单、商品、库存、广告、Ticket 五类数据的跨租户可见性断言。
- 覆盖 API 路由与 Agent 执行链路两条路径，验证 RLS 与业务过滤双重生效。
- 注入反例（错误 tenant context）并验证应返回空结果或权限错误。
- 输出隔离测试报告（失败样本、修复建议、最终通过截图/日志）。

**验收：**
- [ ] AC-P2-15 对应测试全绿
- [ ] 关键表隔离（`products` / `orders` / `inventory_levels` / `ads_campaigns` / `devos_tickets`）均有断言
- [ ] 反例测试可稳定复现并通过
...
**目标：** 搭建并运行 10 租户并发心跳与 Agent 执行压测（AC-P2-16）。  
**对应任务：** 6.2

**关键任务：**
- 实现并发测试驱动器：10 租户 × 5 Agent 的调度与结果采集。
- 记录每租户心跳成功率、平均执行时长、失败原因分布。
- 检查跨租户串写风险（tenantId 漏传、上下文复用、缓存 key 冲突）。
- 增加失败重放机制，支持按 tenant/agent 粒度回放。

**验收：**
- [ ] 10 租户并发场景完成至少 3 轮运行
- [ ] 心跳与执行成功率达到预设阈值（建议 >= 99%）
- [ ] 无数据串混证据
...
##### Day 4 — Amazon 429 限流退避验证

**目标：** 在可控条件下验证 429 自动退避与重试链路（AC-P2-18）。  
**对应任务：** 6.3

**关键任务：**
- 在 `AmazonHarness` 测试中注入 429 响应序列（短突发 + 持续限流两类）。
- 验证指数退避、最大重试次数、最终失败上报 DevOS Ticket 行为。
- 校验队列堆积时的吞吐退化是否可控（无无限重试 / 无队列阻塞）。
- 输出限流参数建议（backoff base、max attempts、jitter）。

**验收：**
- [ ] 429 场景单测与集成测试均通过
- [ ] 退避时间符合策略设计
- [ ] DevOS 上报链路在重试耗尽后触发正确

**产出：** Amazon 限流测试报告 + 参数建议

---

##### Day 5 — 覆盖率提升与薄弱点补测

**目标：** 补齐低覆盖模块，推进新包 >= 70%、总体 >= 80%（AC-P2-17）。  
**对应任务：** 6.5

**关键任务：**
- 生成覆盖率热力图，定位低覆盖文件（优先 harness/market/devos-bridge）。
- 针对错误分支、超时分支、空数据分支补测试。
- 为关键业务规则补守护用例（预算 >$500 审批门控、库存阈值告警、ticket 去重）。
- 建立覆盖率门禁阈值配置并纳入 CI。
...
##### Day 6 — CI 集成扩展（PG + Redis）

**目标：** 让关键集成测试在 CI 自动可重复执行。  
**对应任务：** 6.6

**关键任务：**
- 更新 `.github/workflows`：增加 PostgreSQL 与 Redis services。
- 将 Sprint 6 关键测试拆分为并行 job（隔离 / 并发 / 限流 / 覆盖率）。
- 增加失败工件上传（日志、覆盖率报告、测试快照）。
- 校准 CI 超时与重试策略，避免误报。

**验收：**
- [ ] PR 触发后可自动拉起 PG + Redis 并完成集成测试
- [ ] 失败时可下载可读工件定位问题
- [ ] CI 结果与本地结果一致性通过抽样验证

**产出：** Sprint 6 CI 门禁链路完整可用

---

##### Day 7 — 48h 稳定性运行（阶段一）

**目标：** 启动 48h 长稳运行并监控关键健康指标。  
**对应任务：** 6.4（阶段一）

**关键任务：**
- 启动长稳场景：5 Agent × 多平台，持续心跳执行。
- 监控核心指标：心跳连续性、错误率、队列积压、连接池占用、内存增长。
- 建立告警阈值与值班响应流程（DevOS Ticket 自动流转）。
- 每 6 小时固化一次快照，便于趋势分析。

**验收：**
- [ ] 长稳任务稳定运行超过 24h（阶段性门槛）
- [ ] 关键监控指标可视化完整
- [ ] 告警链路可触发可恢复
...
##### Day 8 — 48h 稳定性运行（阶段二）+ 偏差修正

**目标：** 完成 48h 全周期并处理发现的问题。  
**对应任务：** 6.4（阶段二）

**关键任务：**
- 完成后 24h 运行并形成全程事件时间线。
- 对异常进行分级（P0/P1/P2）与根因归类（代码、配置、外部依赖）。
- 快速修正高优先问题并回归验证关键链路。
- 输出稳定性结论：是否满足 AC-P2-10 的连续运行要求。
...
**产出：** 48h 稳定性最终报告 + 问题闭环清单

---

##### Day 9 — OpenAPI / 运维文档 / ADR 收口

**目标：** 完成对外与对内文档一致性收口。  
**对应任务：** 6.7 / 6.8 / 6.9

**关键任务：**
- 更新 OpenAPI：新平台 OAuth/Webhook、Ads、Inventory、Onboarding 路由。
- 更新运维文档：多平台配置、DevOS 启停、Prometheus 告警与排障步骤。
- 完成 `ADR-0002` 正式稿，固化 Harness 扩展与 Market 层设计决策。
- 执行文档一致性检查（接口字段、错误码、环境变量）。

**验收：**
- [ ] OpenAPI 可生成并通过 lint/validate
- [ ] 运维手册可按步骤在新环境复现
- [ ] ADR-0002 完整且与实现一致

**产出：** Phase 2 文档资产完整可交付

---

##### Day 10 — 20 项验收总闸门 + Sprint Review

**目标：** 完成 AC-P2-01~20 最终验收与 Sprint 关闭。  
**对应任务：** 6.10

**关键任务：**
- 逐项勾验 `§9` 的 20 项 AC，补齐证据链接（测试报告/日志/截图/命令输出）。
- 运行全量门禁：typecheck、lint、unit、integration、coverage、smoke。
- 召开 Sprint Review，沉淀残留风险与 Phase 3 输入 backlog。
- 发布 Sprint 6 关闭记录与下一阶段移交清单。

**验收：**
- [ ] AC-P2-01~20 全部状态明确（✅/⏳/🔽）
- [ ] 全量门禁通过
- [ ] Sprint Review 纪要与移交清单完成

**产出：** Sprint 6 正式关闭包（验收 + 证据 + 移交）

---

#### Sprint 6 · 可复制任务卡（Day 1–Day 10）

> 说明：以下命令默认在仓库根目录执行；如与当前脚本不一致，请按仓库实际脚本名替换。  
> 命名约定：`sprint6:*` 为建议脚本前缀，便于后续统一维护。

---

> **🃏 CARD-D1-01 · 验证基线冻结与测试矩阵落盘**
>
> **类型：** 文档 + 工具脚本  
> **耗时：** 0.5d  
> **目标：** 固化 Sprint 6 可复现基线
>
> **执行步骤：**
> 1. 记录当前 commit、Node、pnpm、PG、Redis 版本。  
> 2. 新建/更新 `docs/ops/sprint6-validation-baseline.md`。  
> 3. 新建/更新 `docs/ops/sprint6-test-matrix.md`，映射 AC-P2-15~20。  
> 4. 建立统一入口脚本（隔离、并发、限流、覆盖率）。
>
> **命令模板：**
> ```bash
> git rev-parse HEAD
...
>
> # 建议新增到 package.json scripts
> pnpm run sprint6:isolation
> pnpm run sprint6:concurrency
> pnpm run sprint6:rate-limit
> pnpm run sprint6:coverage
> ```
>
> **DoD：**
> - [ ] 基线文档与测试矩阵均已提交
...
> **🃏 CARD-D2-01 · 多平台租户隔离测试套件**
>
> **类型：** 集成测试  
> **耗时：** 1d  
> **目标：** AC-P2-15（A/B 租户完全隔离）
>
> **目标文件：**
> - `tests/integration/tenant-isolation.test.ts`
> - `tests/fixtures/tenants/sprint6.ts`
>
> **执行步骤：**
> 1. 构造 Tenant A（Shopify+Amazon）与 Tenant B（TikTok+Shopee）夹具。  
> 2. 对订单、商品、库存、广告、Ticket 做跨租户不可见断言。  
> 3. 注入错误 `tenantId`，验证返回空集合或权限错误。  
> 4. 输出隔离测试报告。
>
> **命令模板：**
> ```bash
> pnpm run sprint6:isolation
> pnpm vitest run tests/integration/tenant-isolation.test.ts
> ```
>
> **DoD：**
> - [ ] 隔离测试全绿
...
>
> **目标文件：**
> - `tests/load/tenants-concurrency.test.ts`
> - `tests/load/runner/tenant-runner.ts`
>
> **执行步骤：**
> 1. 实现 10 租户 × 5 Agent 并发驱动器。  
> 2. 记录每租户成功率、p95 延迟、失败原因。  
> 3. 校验 tenant 上下文隔离与缓存 key 不冲突。  
> 4. 增加失败回放能力（按 tenant/agent 粒度）。
>
> **命令模板：**
> ```bash
> pnpm run sprint6:concurrency
> pnpm vitest run tests/load/tenants-concurrency.test.ts
> ```
>
> **DoD：**
> - [ ] 完成至少 3 轮并发运行
...
>
> **目标文件：**
> - `packages/harness/src/amazon.harness.test.ts`
> - `tests/integration/amazon-rate-limit.test.ts`
>
> **执行步骤：**
> 1. 注入 429 响应序列（突发型/持续型）。  
> 2. 验证指数退避、最大重试次数、jitter。  
> 3. 验证重试耗尽后 DevOS ticket 上报。  
> 4. 校验队列不出现无限重试与阻塞。
>
> **命令模板：**
> ```bash
> pnpm run sprint6:rate-limit
> pnpm vitest run packages/harness/src/amazon.harness.test.ts tests/integration/amazon-rate-limit.test.ts
> ```
>
> **DoD：**
> - [ ] 429 场景测试通过
...
> **🃏 CARD-D5-01 · 覆盖率补测与门禁阈值**
>
> **类型：** 测试工程  
> **耗时：** 1d  
> **目标：** AC-P2-17（新包 >=70%，整体 >=80%）
>
> **执行步骤：**
> 1. 生成覆盖率报告并定位低覆盖模块。  
> 2. 优先补异常分支（超时、空数据、第三方失败）。  
> 3. 增加规则守护用例（预算审批、库存阈值、ticket 去重）。  
> 4. 在 CI 中启用覆盖率阈值失败门禁。
>
> **命令模板：**
> ```bash
...
> **🃏 CARD-D6-01 · CI 扩展 PostgreSQL + Redis**
>
> **类型：** CI/CD  
> **耗时：** 0.5d  
> **目标：** Sprint 6 核心测试自动化
>
> **目标文件：**
> - `.github/workflows/ci.yml`（或等价 workflow）
>
> **执行步骤：**
> 1. 在 workflow 增加 PG 与 Redis services。  
> 2. 拆分 job：isolation / concurrency / rate-limit / coverage。  
> 3. 失败时上传日志与覆盖率工件。  
> 4. 校准 timeout 与重试策略。
>
> **命令模板：**
...
> **🃏 CARD-D7-01 · 48h 稳定性运行启动（24h 检查点）**
>
> **类型：** 稳定性验证  
> **耗时：** 1d（含观察）  
> **目标：** AC-P2-10 阶段一
>
> **执行步骤：**
> 1. 启动 5 Agent × 多平台长稳任务。  
> 2. 采集指标：错误率、队列深度、连接池、内存、CPU。  
> 3. 每 6 小时输出状态快照。  
> 4. 验证告警可触发与自动 Ticket 流转。
>
> **命令模板：**
> ```bash
> pnpm run sprint6:stability:start
> pnpm run sprint6:stability:snapshot
> ```
>
> **DoD：**
> - [ ] 连续运行 >= 24h
...
>
> **执行步骤：**
> 1. 完成 48h 全周期运行并汇总事件时间线。  
> 2. 将异常按 P0/P1/P2 分级并给出根因。  
> 3. 修复高优先问题并执行回归测试。  
> 4. 输出是否可通过长稳验收的结论。
>
> **命令模板：**
> ```bash
> pnpm run sprint6:stability:stop
> pnpm run sprint6:stability:report
> pnpm run sprint6:regression
> ```
>
> **DoD：**
> - [ ] 48h 运行完成
...
> **🃏 CARD-D9-01 · OpenAPI + 运维文档 + ADR 收口**
>
> **类型：** 文档工程  
> **耗时：** 1d  
> **目标：** 6.7 / 6.8 / 6.9 全完成
>
> **目标文件：**
> - `apps/api` 下 OpenAPI 相关文件
> - `docs/ops/*`
> - `docs/adr/0002-*.md`
>
> **执行步骤：**
> 1. 补齐新平台 OAuth/Webhook、Ads、Inventory、Onboarding API。  
> 2. 更新多平台配置与 DevOS 运维手册。  
> 3. 完成 ADR-0002 正式稿并对齐实现。  
> 4. 做字段/错误码/环境变量一致性校验。
>
> **命令模板：**
> ```bash
...
> **🃏 CARD-D10-01 · 20 项 AC 总验收与 Sprint 关闭**
>
> **类型：** 验收 + 项目管理  
> **耗时：** 1d  
> **目标：** 完成 AC-P2-01~20 与 Sprint Review
>
> **执行步骤：**
> 1. 逐项核对 §9 验收条目并链接证据。  
> 2. 跑全量门禁（typecheck/lint/unit/integration/coverage/smoke）。  
> 3. 组织 Sprint Review，沉淀风险与 Phase 3 backlog。  
> 4. 发布 Sprint 6 关闭记录与移交清单。
>
> **命令模板：**
> ```bash
> pnpm typecheck
...
#### Sprint 6 · Day 1–10 卡片执行顺序汇总

**Day 1（基线冻结）**
```text
09:00  CARD-D1-01  基线冻结与测试矩阵落盘                 (3h)
13:30  环境校验      PG/Redis/依赖版本一致性复核             (1h)
15:00  入口联调      sprint6:* 脚本冒烟                      (1h)
16:30  里程碑评审    Day1 DoD 检查                           (30min)
```

**Day 2（隔离测试）**
```text
09:00  CARD-D2-01  租户 A/B 夹具与隔离断言                 (3h)
13:30  反例注入      错误 tenant context 验证                 (1.5h)
15:30  报告整理      AC-P2-15 证据沉淀                       (1h)
16:45  日终复盘      Day2 DoD 检查                           (15min)
```

**Day 3（并发测试）**
```text
09:00  CARD-D3-01  10 租户并发驱动器实现                   (3h)
13:30  三轮压测      成功率/延迟/串混风险分析                 (2h)
15:45  回放机制      失败样本按 tenant/agent 重放             (1h)
17:00  日终复盘      Day3 DoD 检查                           (15min)
```

**Day 4（429 限流）**
```text
09:00  CARD-D4-01  429 场景注入与退避参数验证              (2.5h)
13:30  重试耗尽链路   DevOS Ticket 上报验证                  (1.5h)
15:30  回归冒烟      Amazon 关键链路回归                     (1h)
16:45  日终复盘      Day4 DoD 检查                           (15min)
```

**Day 5（覆盖率）**
```text
09:00  CARD-D5-01  覆盖率热力图与薄弱点定位                (2h)
11:30  分支补测      异常/超时/空数据分支补齐                (2.5h)
15:00  规则守护用例   审批门控/库存阈值/ticket 去重           (1.5h)
16:45  日终复盘      Day5 DoD 检查                           (15min)
```

**Day 6（CI 扩展）**
```text
09:00  CARD-D6-01  Workflow 增加 PG + Redis services       (2h)
11:30  Job 拆分      isolation/concurrency/rate-limit/coverage (2h)
14:30  工件上传      日志与覆盖率报告归档                     (1h)
16:00  一致性抽样     本地 vs CI 结果对比                     (45min)
16:45  日终复盘      Day6 DoD 检查                           (15min)
```

**Day 7（48h 稳定性 · 上半程）**
```text
09:00  CARD-D7-01  48h 任务启动与指标面板确认               (2h)
11:30  快照 1        关键指标第一次落盘                       (30min)
15:30  快照 2        队列/连接池/错误率趋势检查                (30min)
17:00  阶段评审      >=24h 连续运行目标跟踪                   (30min)
```

**Day 8（48h 稳定性 · 下半程）**
```text
09:00  CARD-D8-01  异常分级与根因归类                        (2h)
11:30  高优先修复     P0/P1 问题闭环                           (2h)
14:30  回归验证      关键链路回归                              (1.5h)
16:30  稳定性结论     48h 最终报告输出                         (30min)
```

**Day 9（文档收口）**
```text
09:00  CARD-D9-01  OpenAPI 更新与校验                        (2h)
11:30  运维文档补齐    多平台 + DevOS + 告警排障                (2h)
14:30  ADR-0002 收口  决策与实现一致性核对                      (1.5h)
16:30  文档走查      字段/错误码/环境变量一致性检查              (30min)
```

**Day 10（总验收关闭）**
```text
09:00  CARD-D10-01 AC-P2-01~20 逐项验收                      (2.5h)
13:00  全量门禁      typecheck/lint/test/coverage/smoke       (2.5h)
15:45  Sprint Review 风险复盘 + Phase 3 backlog              (1h)
17:00  关闭发布      Sprint 6 关闭记录与移交清单               (30min)
```

---

#### Sprint 6 · 每日产出汇总（周会总览）

```text
Day 1  │ 基线冻结 ✅ │ 测试矩阵 ✅ │ sprint6:* 入口脚本 ✅
Day 2  │ 租户隔离测试 ✅ │ A/B 反例验证 ✅ │ AC-P2-15 证据归档 ✅
Day 3  │ 10 租户并发驱动 ✅ │ 3 轮压测 ✅ │ 无串混结论 ✅
Day 4  │ Amazon 429 注入 ✅ │ 退避重试验证 ✅ │ AC-P2-18 证据归档 ✅
Day 5  │ 覆盖率补测 ✅ │ 新包 >=70% ✅ │ 总体 >=80% ✅
Day 6  │ CI 增加 PG+Redis ✅ │ 并行 Job ✅ │ 失败工件上传 ✅
Day 7  │ 48h 稳定性启动 ✅ │ 24h 检查点 ✅ │ 监控快照持续输出 ✅
Day 8  │ 48h 稳定性完成 ✅ │ 高优先问题闭环 ✅ │ 回归通过 ✅
Day 9  │ OpenAPI 收口 ✅ │ 运维文档更新 ✅ │ ADR-0002 定稿 ✅
Day 10 │ AC-P2-01~20 总验收 ✅ │ 全量门禁 ✅ │ Sprint 6 关闭发布 ✅
```

**Sprint 6 里程碑检查点：**
- M1（Day 2 结束）：AC-P2-15 已可证明。
- M2（Day 4 结束）：AC-P2-18 已可证明。
- M3（Day 6 结束）：CI 集成门禁稳定运行。
- M4（Day 8 结束）：48h 稳定性结果可用于验收。
- M5（Day 10 结束）：AC-P2-01~20 全量状态清晰并完成关闭。
...
|---|---|---|---|---|---|
| 48h 稳定性运行中断（AC-P2-10） | A | 连续 2 次快照错误率 > 1% 或任务中断 > 10min | 启用快照告警 + 异常分级，Day 8 前闭环 P0/P1 | SRE / Agent Runtime | Day 7 17:00 |
| 10 租户并发出现串混（AC-P2-16） | A | 任意跨租户读写命中 1 次即升 R | 强制 tenant context 断言 + 失败回放 + 缓存 key 审计 | Backend | Day 3 17:00 |
| Amazon 429 退避策略不足（AC-P2-18） | A | 重试耗尽率 > 5% 或队列 backlog 持续增长 2h | 调整 backoff/jitter/max attempts，补回归用例 | Harness | Day 4 16:45 |
| 覆盖率不达标（AC-P2-17） | A | 新包 < 70% 或总体 < 80% | 热力图补测 + CI 阈值门禁 | QA / Dev | Day 5 16:45 |
| CI 集成环境不稳定（6.6） | G | 同一 commit 的 CI 与本地结果不一致 >= 2 次 | 固化 PG/Redis 版本与 timeout，保留失败工件 | DevOps | Day 6 16:45 |
| 文档与实现不一致（6.7/6.8/6.9） | G | OpenAPI 校验失败、ADR 字段与代码不一致 | Day 9 执行字段/错误码/env 三项对齐走查 | Tech Lead | Day 9 16:30 |
| 20 项 AC 证据链不完整（6.10） | A | 任一 AC 无可追溯证据链接 | Day 10 前建立 AC→证据索引表并预审 | PM / QA | Day 10 09:00 |
...
#### Sprint 6 · 每日报数模板（昨日/今日/阻塞）

> 用法：每日站会前复制一份，5-10 分钟内填完；证据尽量链接到测试报告、CI 记录或文档 PR。

```markdown
### Sprint 6 Day X 日报（YYYY-MM-DD）
...
**快速判定规则（30 秒）：**
- `G`：当天 DoD 基本可按时完成，无关键阻塞。
- `A`：存在阻塞但 24h 内可消解，不影响 Sprint 主路径。
- `R`：已影响关键路径或无法在 24h 内恢复，需要升级处理。

**升级触发（建议）：**
- 同一阻塞连续 2 日未关闭；
- AC 关键项（15/16/17/18/10）任一出现回退；
- CI 与本地结果连续 2 次不一致。

---

#### Sprint 6 · 结束复盘模板（Retro）

> 用法：Day 10 Sprint Review 后 30-45 分钟填写；行动项必须可追踪到 owner 和截止日期。

```markdown
### Sprint 6 复盘（YYYY-MM-DD）

#### A. 结果概览
- Sprint 目标达成率：`xx%`
- AC 通过情况：`通过 x/20，待补 x，降级 x`
- 总体结论：`达成 / 基本达成 / 未达成`
...
|---|---|---|---|---|---|
| 1 | `...` | `@name` | `YYYY-MM-DD` | `可验证标准` | `AC-P2-xx / RAG-xx` |
| 2 | `...` | `@name` | `YYYY-MM-DD` | `可验证标准` | `AC-P2-xx / RAG-xx` |
| 3 | `...` | `@name` | `YYYY-MM-DD` | `可验证标准` | `AC-P2-xx / RAG-xx` |
...
- [ ] 每条行动项有可验证验收标准
- [ ] 关联到至少 1 个 AC 或 1 项风险
- [ ] Phase 3 移交项均有链接证据

---

#### Sprint 6 · 指标看板模板（Monitoring KPI）

> 用法：Day 1 起每日更新 1 次；Day 7-8 稳定性阶段建议每 6 小时更新一次。  
> 建议落地文件：`docs/ops/sprint6-metrics-dashboard.md`
...
| 429 重试耗尽率 | < 1% | 1% ~ 5% | > 5% | Harness 指标 |
| 并发串混事件数 | 0 | 1（待确认） | >= 1（已确认） | 隔离测试/审计日志 |
| CI 本地一致性偏差 | 0 次 | 1 次 | >= 2 次 | PR 记录 |

**日报表（可复制）：**

```markdown
### Sprint 6 指标日报（YYYY-MM-DD HH:mm）
...
- `R`：立即升级到 Sprint 风险看板红项，触发专项处理与节奏调整。

---

#### Sprint 6 · 一页式指挥板（Daily Command Board）

> 用法：每天开工前 5 分钟更新，站会时仅看这一页。  
> 目标：所有人快速对齐「今天最重要的事、最大风险、必须决策」。

```markdown
### Sprint 6 指挥板（YYYY-MM-DD）
...
|---|---|---|---|---|---|
| P1 | `...` | `CARD-Dx-01 / AC-P2-xx` | `@...` | `HH:mm` | `未开始/进行中/已完成` |
| P2 | `...` | `...` | `@...` | `HH:mm` | `...` |
| P3 | `...` | `...` | `@...` | `HH:mm` | `...` |
| P4 | `...` | `...` | `@...` | `HH:mm` | `...` |
| P5 | `...` | `...` | `@...` | `HH:mm` | `...` |
...
- 若存在 `R` 风险，必须同步一个明确兜底动作与 owner。
- 待决策项超过 2 个时，说明计划分歧过大，需临时评审。

#### Sprint 6 · 模板引用入口

为避免在大文档内反复拷贝，Sprint 6 日常协作统一使用下列模板：

- 日报模板：`docs/templates/sprint-daily-report-template.md`
- 风险看板：`docs/templates/sprint-risk-rag-template.md`
- 一页式指挥板：`docs/templates/sprint-command-board-template.md`
- 指标看板：`docs/templates/sprint-metrics-dashboard-template.md`
- 结束复盘：`docs/templates/sprint-retro-template.md`
