...
export class AutonomousDevLoop {
   ...
   * Returns the LoopRunSummary on completion.
   * Throws LoopError on unrecoverable failures.
   */
  async run(ticket: DevOsTicket, runId: string): Promise<LoopRunSummary> {
    ...
    try {
      ...
      try {
        ...
        ctx.failStage(4, msg)
        throw new LoopError('task_graph_cycle', {
      ...
      if (failedTasks.length > 0) {
        ...
        throw new LoopError('deployment_failed', {
          stage: 5,
          ticketId: ticket.context.agentId,
      ...
      // AC-P4-02: coverage < 80% → LoopError("coverage_below_80")
      // AC-P4-03: security issues → LoopError("security_issues")
      ...
      for (let attempt = 1; attempt <= this.maxCodeReviewRetries; attempt++) {
        ...
        if (!qa.passed || qa.coveragePct < 80) {
          ...
          if (attempt >= this.maxCodeReviewRetries) {
            throw new LoopError('coverage_below_80', {
              stage: 6,
              ticketId: ticket.context.agentId,
        ...
        if (!sec.passed || sec.vulnerabilities.length > 0) {
          ...
          if (attempt >= this.maxCodeReviewRetries) {
            throw new LoopError('security_issues', {
              stage: 6,
              ticketId: ticket.context.agentId,
              vulnerabilities: sec.vulnerabilities.length,
            })
      ...
      try {
        ...
        throw new LoopError(code, { stage: 7, ticketId: ticket.context.agentId, details: msg })
      }
      ...
      if (!sreResult.healthy) {
        ...
        throw new LoopError('health_check_failed', {
          stage: 9,
          ticketId: ticket.context.agentId,
      ...
      if (err instanceof LoopError) return ctx.getSummary()
      throw err
    }
  }
}
