...
const tikTokOAuthRoute: FastifyPluginAsync = async (app) => {
  ...
  app.get('/api/v1/tiktok/auth', {
    schema: {
      tags: ['TikTok OAuth'],
      summary: 'Start TikTok OAuth for the current tenant',
      security: [{ bearerAuth: [] }],
    },
  }, async (request, reply) => {
    ...
    const state = signOAuthState({ tenantId, appKey, shopId }, stateSecret)
  ...
  app.get('/api/v1/tiktok/auth/callback', {
    schema: {
      tags: ['TikTok OAuth'],
      summary: 'Complete TikTok OAuth callback for the current tenant',
      security: [{ bearerAuth: [] }],
    },
  }, async (request, reply) => {
    const appSecret = process.env.TIKTOK_APP_SECRET
    ...
    const statePayload = verifyOAuthState<TikTokStatePayload>(
      query.state ?? '',
      stateSecret,
    )
    if (!statePayload) {
